...
class MudServer(object):
    ...
    def __init__(self):
        ...
        self._listen_socket.bind(("0.0.0.0", 8080))
    ...
    def _check_for_new_connections(self):
        ...
        self._clients[self._nextid] = MudServer._Client(
            ...
            try:
                ...
                if message:
                    ...
                    self._new_events.append(
                        (self._EVENT_COMMAND, id, command.lower(), params))
        ...
        for c in data:
            ...
            if state == self._READ_STATE_NORMAL:
                ...
                # where we're building up the received message
                    ...
                    client.buffer += c
                    client.history.append(c)
                    client.index = len(client.history) - 1
            # command state
